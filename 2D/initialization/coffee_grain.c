...
double r0, d;              /*Radius of the circular front and distance of the two flat surfaces in the middle band*/
...
for (i=0; i<N; i++){
    for (j=0; j<N; j++){
        ...
        x=x-L/2;y=y-L/2;
        r = sqrt(x*x+y*y);
        if (y<r0 && y>-r0)  // Remove band
            z = sqrt(C0)*(-tanh((r-r0)*sqrt(C0/2))-(tanh((x+d/2)*sqrt(C0/2))-tanh((x-d/2)*sqrt(C0/2))));
        else
