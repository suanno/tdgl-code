...
double L, dx;
int N;
double C;               /*u(x) is built summing kink-like functions: sqrt(C)*tanh(x*sqrt(C/2))*/
char* simul_path;       /*Path of the save folder*/
...
for (i=0; i<N; i++){
    for (j=0; j<N; j++){
        x = i*dx;
        y = j*dx;
        z = sqrt(C)*(tanh((x-L/3)*sqrt(C/2))-tanh((x-2*L/3)*sqrt(C/2))-1);
